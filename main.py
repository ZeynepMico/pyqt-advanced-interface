...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Grafik verilerini 8 değer için yeniden yapılandır
        self.x_data = []
        self.y_data = [[] for _ in range(8)] # 8 adet Y ekseni veri listesi
        self.time_index = 0
        ...
        self.parsing_config = {
            "mode": "simple",
            "delimiter": ",",
            "columns": list(range(1, 9)), # 8 sütun için liste
            "regex_pattern": r"(-?\d+\.?\d*)" * 8 # 8 grup yakalamak için örnek
        }
    ...
    def apply_access_control(self):
        ...
        if pg:
            ...
            self.pushButtonExportPlot.setVisible(False)
        ...
        self.pushButtonResetLog.setVisible(is_admin)
        self.pushButtonResetTerminal.setVisible(is_admin)

        if pg:
            self.pushButtonResetGraph.setVisible(is_admin)
        else:
            if hasattr(self, 'pushButtonResetGraph'):
                self.pushButtonResetGraph.setVisible(False)
                
        # SSH Reset butonu admin ve user tarafından görülebilir
        is_admin_or_user = is_admin or (self.user_role == "user")
        self.pushButtonResetSshTerminal.setVisible(is_admin_or_user)
        ...
        if pg:
            ...
            for chk in self.chk_curves:
                chk.setEnabled(graph_controls_enabled)

            self.chk_show_legend.setEnabled(graph_controls_enabled)
            self.btn_apply_plot_settings.setEnabled(graph_controls_enabled)
    
    def openDataParserSettings(self):
        dialog = DataParserDialog(self.parsing_config, self)
        dialog.settingsApplied.connect(self.apply_parser_settings)  
        dialog.exec_()
    ...
    def toggle_theme(self, checked):
        ...
        self.theme_switch_checkbox.setText("Switch to Dark Theme" if checked else "Switch to Open Theme")
    ...
    def apply_theme(self, is_dark_mode):
        ...
        dark_stylesheet = """
            QWidget {
                background-color: #2E2E2E; /* Ana arka plan - Koyu Gri */
                color: #E0E0E0; /* Ana metin rengi - Açık Gri */
                font-family: 'Segoe UI'; font-size: 10pt;
            }
            QGroupBox {
                border: 1px solid #555555; /* Grup kutusu kenarlığı - Gri */
            ...
            QGroupBox::title {
                ...
                color: #CCCCCC; /* Başlık rengi - Açık Gri */
            }
            /* Terminal alanları HARİÇ diğer inputlar */
            QLineEdit, QListWidget, QSpinBox, QComboBox {
                background-color: #3C3C3C; /* Input arka planı - Orta Gri */
                border: 1px solid #666666; /* Input kenarlığı */
                border-radius: 5px;
                padding: 5px;
                color: #E0E0E0;
            }
            QLineEdit:focus, QSpinBox:focus, QComboBox:focus { 
                border: 1px solid #BBBBBB; /* Odaklanınca kenarlık */
            }
            QPushButton {
                background-color: #5A5A5A; /* Buton rengi - Gri */
                color: #FFFFFF; /* Buton yazı rengi - Beyaz */
                border: none; padding: 8px 15px;
                border-radius: 5px; font-weight: bold;
            }
            QPushButton:hover { background-color: #6B6B6B; } /* Üzerine gelince */
            QPushButton:pressed { background-color: #494949; } /* Basılınca */
            QPushButton:disabled { background-color: #404040; color: #888888; }
            QPushButton:checked { background-color: #007ACC; } /* Stop butonu gibi seçili butonlar için mavi */
            QTabBar::tab {
                background: #3C3C3C; border: none; padding: 8px 12px;
                border-top-left-radius: 5px; border-top-right-radius: 5px;
                color: #AAAAAA; /* Pasif sekme rengi */
                font-weight: bold;
            }
            /* Seçili sekme beyaz renkte vurgulanacak */
            QTabBar::tab:selected { 
                background: #2E2E2E; color: #FFFFFF; border-top: 2px solid #FFFFFF; 
            }
        ...
        terminal_style = "background-color: #000000; color: #E0E0E0;"
        self.textEditReceiveData.setStyleSheet(terminal_style)
        self.terminalTextEdit.setStyleSheet(terminal_style)
        self.sshTerminalTextEdit.setStyleSheet(terminal_style)

        if pg and hasattr(self, 'plot_widget'):
            if is_dark_mode:
                self.plot_widget.setBackground('#2E2E2E') # Koyu tema ana arka planı
                axis_pen = pg.mkPen(color='#E0E0E0')      # Koyu tema ana metin rengi
            else:
    ...
    def initUI(self):
        ...
        # Grup 3: Sadece Veri Akışı Kontrolü
        self.pushButtonStop = QPushButton("Stop")
        self.pushButtonStop.setIcon(style.standardIcon(QStyle.SP_MediaStop))
        ...
        self.tab1 = QWidget()
        self.tab_widget.addTab(self.tab1, "System Log")
        ...
        tab1_controls_layout = QHBoxLayout()
        self.pushButtonSaveData = QPushButton("Save Raw Data Log")
        self.pushButtonSaveData.setIcon(style.standardIcon(QStyle.SP_DialogSaveButton))
        self.pushButtonSaveData.setEnabled(False) # Bağlantı kurulunca aktif olacak
        self.pushButtonResetLog = QPushButton("Clear Log")
        self.pushButtonResetLog.setIcon(style.standardIcon(QStyle.SP_TrashIcon))
        tab1_controls_layout.addStretch() # Butonları sağa yasla
        tab1_controls_layout.addWidget(self.pushButtonSaveData)
        tab1_controls_layout.addWidget(self.pushButtonResetLog)
        tab1_layout.addLayout(tab1_controls_layout)
        ...
        if pg:
            ...
            # 8 eğri ve 8 renk tanımla
            self.curves = []
            self.chk_curves = []
            self.plot_colors = ['r', 'g', 'b', 'c', 'm', 'y', (255,165,0), (128,0,128)] # 8 renk
            
            for i in range(8):
                pen = pg.mkPen(color=self.plot_colors[i], width=2)
                curve = self.plot_widget.plot(pen=pen, name=f"Data-{i+1}")
            ...
            group_curve_layout = QGridLayout(group_curve_visibility) # Grid layout for checkboxes
            
            for i in range(8):
                chk = QCheckBox(f"Data-{i+1}")
                chk.setChecked(True)
                # Lambda'da 'index=i' kullanarak doğru index'in yakalanmasını sağla
                chk.stateChanged.connect(lambda state, index=i: self.curves[index].setVisible(state == Qt.Checked))
                self.chk_curves.append(chk)
                group_curve_layout.addWidget(chk, i // 2, i % 2) # 4x2 grid
            ...
            tab2_controls_layout = QHBoxLayout()
            self.pushButtonExportPlot = QPushButton("Export Plot")
            self.pushButtonExportPlot.setIcon(style.standardIcon(QStyle.SP_FileIcon))
            self.pushButtonResetGraph = QPushButton("Reset Graph")
            self.pushButtonResetGraph.setIcon(style.standardIcon(QStyle.SP_TrashIcon))
            tab2_controls_layout.addStretch() # Butonları sağa yasla
            tab2_controls_layout.addWidget(self.pushButtonExportPlot)
            tab2_controls_layout.addWidget(self.pushButtonResetGraph)
            tab2_layout.addLayout(tab2_controls_layout, 1, 0, 1, 3)
        ...
        tab3_controls_layout = QHBoxLayout()
        self.pushButtonSaveTerminal = QPushButton("Save Terminal Log")
        self.pushButtonSaveTerminal.setIcon(style.standardIcon(QStyle.SP_DialogSaveButton))
        self.pushButtonResetTerminal = QPushButton("Clear Terminal")
        self.pushButtonResetTerminal.setIcon(style.standardIcon(QStyle.SP_TrashIcon))
        tab3_controls_layout.addStretch() # Butonları sağa yasla
        tab3_controls_layout.addWidget(self.pushButtonSaveTerminal)
        tab3_controls_layout.addWidget(self.pushButtonResetTerminal)
        tab3_layout.addLayout(tab3_controls_layout)
        ...
        self.pushButtonResetSshTerminal = QPushButton("Clear")
        self.pushButtonResetSshTerminal.setIcon(style.standardIcon(QStyle.SP_TrashIcon))
        ssh_send_layout.addWidget(self.pushButtonResetSshTerminal)
        ...
        self.pushButtonResetLog.clicked.connect(self.reset_log_data)
        if pg: self.pushButtonExportPlot.clicked.connect(self.export_plot)
        self.pushButtonResetGraph.clicked.connect(self.reset_graph_data)
        
        self.pushButtonSaveTerminal.clicked.connect(self.save_terminal_data_to_file)
        self.pushButtonResetTerminal.clicked.connect(self.reset_terminal_data)
        
        self.pushButtonResetSshTerminal.clicked.connect(self.reset_ssh_terminal_data)
    ...
    def update_plot_data(self, values):
        if not pg or len(values) != 8: return
        ...
        # Eğrileri yeni verilerle güncelle
        for i in range(8):
            self.curves[i].setData(self.x_data, self.y_data[i])

        # Otomatik Y ekseni ayarı
        if self.plot_auto_range:
            all_visible_y = []
            for i in range(8):
                if self.chk_curves[i].isChecked() and self.y_data[i]:
                    all_visible_y.extend(self.y_data[i][-max_points:])
            
            if all_visible_y:
                 min_y = min(all_visible_y)
                 max_y = max(all_visible_y)
                 padding = (max_y - min_y) * 0.1 if max_y > min_y else 5
                 self.plot_widget.setYRange(min_y - padding, max_y + padding)
            # Otomatik X ekseni kaydırma
    ...
    def logout_and_show_login(self):
        ...
        self.reset_terminal_data()
        self.reset_ssh_terminal_data()
    ...
    def apply_parser_settings(self, config):
        self.parsing_config = config
        print("Yeni parser ayarı uygulandı:", self.parsing_config)
    ...
    def disconnect_ssh(self):
        ...
        if self.ssh_worker:
            ...
            # Bağlantı kesilince SFTP listesini temizle
            self.sftpFileListWidget.clear()
        ...
        # UI'yı başlangıç durumuna döndür
        self.sshConnectButton.setEnabled(True)
        self.sshDisconnectButton.setEnabled(False)
        self.sshHostInput.setEnabled(True)
        self.sshPortInput.setEnabled(True)
    ...
    # Sekme Bazlı Reset Fonksiyonları 
    def reset_log_data(self):
        """Sadece 'Received Data (Log)' sekmesini ve ilgili verileri temizler."""
        self.recorded_lines.clear()
        self.textEditReceiveData.clear()
        self.append_to_log("Raw data log has been cleared.", color='blue')

    def reset_graph_data(self):
        """Sadece grafik verilerini ve çizimi sıfırlar."""
        if pg:
            self.x_data.clear()
            for i in range(8):
                self.y_data[i].clear()
            ...
            self.time_index = 0
            self.plot_auto_range = True # Otomatik modu tekrar aktifleştir
            self.plot_widget.setXRange(0, 100)
            self.plot_widget.setYRange(0, 100)
            self.append_to_log("Graph data has been reset. Auto-ranging is ON.", color='blue')
            
    def reset_terminal_data(self):
        """Sadece seri port terminalini ve komut geçmişini temizler."""
        self.terminalTextEdit.clear()
        self.terminal_command_history.clear()
        self.terminal_history_index = -1
        self.append_to_log("Serial terminal has been cleared.", color='blue')
        
    def reset_ssh_terminal_data(self):
        """Sadece SSH terminalini ve komut geçmişini temizler."""
        self.sshTerminalTextEdit.clear()
        self.ssh_command_history.clear()
        self.ssh_history_index = -1
        self.append_to_log("SSH terminal has been cleared.", color='blue')
    ...
    def portDataReceived(self):
        """Seri porttan veri geldiğinde tetiklenir."""
        if not self.serialPort.isOpen():
            return
        
        # Gelen tüm byte'ları oku ve tampona ekle
        received_data = self.serialPort.readAll()
        self.received_buffer.extend(received_data)

        mode = self.parsing_config.get("mode", "simple")

        # Binary ve Packet modları için döngüsel işleme
        processing = True
        while processing:
            gps_data = None
            bytes_consumed = 0
            ...
            if mode == "binary":
                gps_data, bytes_consumed = self.parse_gps_binary(self.received_buffer)
            elif mode == "packet":
                gps_data, bytes_consumed = self.parse_gps_packet(self.received_buffer)
            
            # Eğer binary/packet modundaysak ve veri işlendiyse
            if bytes_consumed > 0:
                if gps_data:
                    self.handle_gps_data(gps_data)
                # Tüketilen byte'ları tampondan kaldır
                self.received_buffer = self.received_buffer[bytes_consumed:]
                processing = True
            else:
                processing = False

        # Metin tabanlı modlar için (eski işleyiş korunuyor)
        if mode in ["simple", "regex"]:
            try:
                text_data = self.received_buffer.decode('utf-8', errors='ignore')
                if '\n' in text_data:
                    lines = text_data.split('\n')
                    self.received_buffer = bytearray(lines[-1].encode('utf-8'))
                    ...
                    for line in lines[:-1]:
                        ...
                        # Ham veriyi logla
                        self.append_to_log(f"RAW: {clean_line}", color='black')
                        self.recorded_lines.append(clean_line)
                        
                        if clean_line.startswith('$GPRMC'):
                            self.parse_nmea_sentence(clean_line)
                        else:
                            self.parse_other_data(clean_line)
                else:
                    self.received_buffer = text_data.encode('utf-8')
            except UnicodeDecodeError:
                self.append_to_log("Warning: Received non-UTF8 data in text mode.", color='orange')
    ...
    def terminal_lineEdit_key_press_event(self, event):
        ...
        if event.key() == Qt.Key_Up:
            ...
            QLineEdit.keyPressEvent(self.terminalLineEdit, event)

    def parse_other_data(self, clean_line):
        """
        NMEA olmayan diğer metin tabanlı verileri işler. 
        Gelen veri adedi kadar değeri alır, geri kalanını 'çizilmemesi için' NaN olarak ayarlar.
        """
        values = []
        try:
            # Mod 1: Basit Ayrıştırıcı
            if self.parsing_config["mode"] == "simple":
                parts = clean_line.split(self.parsing_config["delimiter"])
                columns_to_parse = self.parsing_config.get("columns", [])
                
                # Gelen veriyi ayrıştır
                parsed_values = []
                for col_num in columns_to_parse:
                    col_index = col_num - 1
                    if col_index < len(parts):
                        parsed_values.append(float(parts[col_index].strip()))
                    else:
                        break # Eğer istenen sütun yoksa daha fazla arama

                # Nihai listeyi oluştur: bulunanları ekle, geri kalanını NaN ile doldur
                for i in range(8):
                    if i < len(parsed_values):
                        values.append(parsed_values[i])
                    else:
                        values.append(float('nan')) # Çizilmemesi için NaN ata
                
            # Mod 2: Regex Ayrıştırıcı
            elif self.parsing_config["mode"] == "regex": 
                pattern = self.parsing_config.get("regex_pattern", "")
                if not pattern: return
                match = re.search(pattern, clean_line)
                
                parsed_values = []
                if match:
                    parsed_values = [float(g) for g in match.groups()]
            ...
            if values:
                valid_values_str = ", ".join([f"D{i+1}={v:.2f}" for i, v in enumerate(values) if v is not float('nan')])
                if valid_values_str:
                     self.append_to_terminal(f"Graph Data: {valid_values_str}", color="#2A7E22")

                if pg: self.update_plot_data(values)
                return

            line_lower = clean_line.lower()
            if "led yandi" in line_lower:
                self.append_to_terminal("STM32 LED Açıldı.", color='green')
            elif "led sondu" in line_lower:
    ...
    def apply_plot_settings(self):
        ...
        for i in range(8):
            self.curves[i].setVisible(self.chk_curves[i].isChecked())
    ...
    def crc16(self, data: bytes) -> int:
        """Verilen C koduna göre CRC16 hesaplar."""
        crc = 0
        for b in data:
            crc ^= (b << 8)
            for _ in range(8):
    ...
    def parse_gps_binary(self, buffer: bytearray):
        """
        Buffer içinde kayarak geçerli bir 48 byte'lık GPS veri çerçevesi arar.
        Bulduğu verinin geçerli koordinatlar içerip içermediğini kontrol eder. 
        """
        for i in range(len(buffer) - 47):
            aday_cerceve = buffer[i : i + 48]
            try:
                # unpack fonksiyonunu struct modülü üzerinden çağırıyoruz
                degerler = struct.unpack("<6d", aday_cerceve)

                h_lat, h_lon = degerler[0], degerler[1]
                t_lat, t_lon = degerler[3], degerler[4]

                if not (-90 <= h_lat <= 90 and -180 <= h_lon <= 180 and \
                        -90 <= t_lat <= 90 and -180 <= t_lon <= 180):
                    continue

                gps_data = {
                    "h_lat": degerler[0], "h_lon": degerler[1], "h_alt": degerler[2],
                    "t_lat": degerler[3], "t_lon": degerler[4], "t_alt": degerler[5]
                }
                return gps_data, i + 48

            except struct.error:
                continue
        return None, 0

    def parse_gps_packet(self, buffer: bytearray):
        """
        Header, ID, Len, Payload, CRC içeren paketi doğrular ve ayrıştırır.
        """
        start_index = buffer.find(b'\x55\xAA')
        if start_index == -1:
            return None, 0

        if len(buffer) < start_index + 54:
            return None, 0
            
        packet_candidate = buffer[start_index : start_index + 54]
        payload_len = packet_candidate[3]

        if payload_len != 48:
            self.append_to_log(f"Packet Error: Invalid payload length {payload_len}. Expected 48.", color='red')
            return None, start_index + 1

        payload = packet_candidate[4 : 4 + payload_len]
        received_crc = int.from_bytes(packet_candidate[52:54], 'big')
        crc_data = packet_candidate[2:52]
        calculated_crc = self.crc16(crc_data)

        if received_crc != calculated_crc:
            self.append_to_log(f"CRC Mismatch! Received: {received_crc}, Calculated: {calculated_crc}", color='red')
            return None, start_index + 1
        
        gps_data, _ = self.parse_gps_binary(payload) # Payload'u işlemek için zaten akıllı olan binary parser'ı kullan
        return gps_data, start_index + 54

    def handle_gps_data(self, gps_data: dict):
        """Ayrıştırılmış GPS verisini alır ve harita ile grafiği günceller."""
        
        if not gps_data or not all(math.isfinite(v) for v in gps_data.values()):
            self.append_to_log(f"Geçersiz veri (nan/inf) alındı, işlenmiyor: {gps_data}", color='red')
            return 

        h_lat = gps_data.get('h_lat')
        h_lon = gps_data.get('h_lon')
        t_lat = gps_data.get('t_lat')
        t_lon = gps_data.get('t_lon')

        log_msg = (f"GPS Data Parsed -> Home: ({h_lat:.6f}, {h_lon:.6f}), "
                   f"Target: ({t_lat:.6f}, {t_lon:.6f})")
        self.append_to_log(log_msg, color="purple")
        
        if hasattr(self, 'map_tab'):
            self.map_tab.move_marker(h_lat, h_lon)
            self.map_tab.add_point_to_live_route(h_lat, h_lon)
            self.map_tab.update_target_fields(t_lat, t_lon)
            self.map_tab.mapLatInput.setText(str(h_lat))
            self.map_tab.mapLonInput.setText(str(h_lon))
            self.map_tab.homeAltInput.setText(str(gps_data.get('h_alt', 0)))
            self.map_tab.targetAltInput.setText(str(gps_data.get('t_alt', 0)))
            self.map_tab.calculate_distance_and_draw_route()

        if pg:
            gps_values_for_plot = [
                h_lat, h_lon, gps_data.get('h_alt', 0),
                t_lat, t_lon, gps_data.get('t_alt', 0),
                float('nan'), float('nan')
            ]
            self.update_plot_data(gps_values_for_plot)
